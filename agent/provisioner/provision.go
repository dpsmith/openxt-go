...
func NewConfig(path string) (*Config, error) {
	...
	if c.Functions == nil {
		c.Functions = make(ShellFuncList)
	}
	if c.Answers == nil {
		c.Answers = make(*AnswerFile)
	}
	if c.Distros == nil {
		c.Distros = make(*Distro)
	}
	if c.Systems == nil {
		c.Systems = make(*System)
	}
	return &c, nil
}
...
func (p *Provisioner) syncAnswers() error {
	...
	for n, a := range p.config.Answers {
		if err := a.WriteFile(p.config.HttpPath, &p.config.Functions); err != nil {
